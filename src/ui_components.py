    ...
    def _handle_predefined_speaker(self, tts):
        ...
        if available_speakers:
            selected_speaker = st.selectbox(
                "Select a predefined speaker:", available_speakers)
            return selected_speaker, None
        else:
            st.warning(
                "No predefined speakers available. Please upload your own voice.")
            return None, None

    def _handle_voice_upload(self):
        """Handle voice upload functionality"""
        st.info(
            "Upload a clear audio file of your voice (WAV format recommended, 3-10 seconds)")
        uploaded_file = st.file_uploader(
            "Upload your voice sample", type=["wav", "mp3"])
